...
const SECTORS = [
  'Informatique et télécoms', 'BTP et travaux publics', 'Santé et médical',
  'Éducation et formation', 'Énergie et environnement', 'Transport et logistique',
  'Services aux entreprises', 'Agriculture & Agroalimentaire',
]

const REGIONS = [
  'Casablanca-Settat', 'Rabat-Salé-Kénitra', 'Marrakech-Safi',
  'Souss-Massa', 'Fès-Meknès', 'Tanger-Tétouan-Al Hoceïma',
...
export default function AlertesPage() {
  const [rules, setRules] = useState<AlertRule[]>([
    ...
    { id: 2, type: 'sector', value: 'BTP et travaux publics', active: true, frequency: 'DAILY' },
    ...
    <>
      <div className="u-page-header">
        <div className="u-page-title">
          ...
          <p>{activeCount} alerte{activeCount !== 1 ? 's' : ''} active{activeCount !== 1 ? 's' : ''} — Vous recevez des notifications dès qu&apos;un marché correspond</p>
        </div>
      </div>
      ...
      <div className="u-stats-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
        <div className="u-stat-card">
          ...
          <div className="u-stat-body">
            ...
            <div className="u-stat-sub blue"><i className="fa-solid fa-circle-check" aria-hidden /> Sur {rules.length} configurées</div>
          </div>
        </div>
        <div className="u-stat-card">
          <div className="u-stat-icon green"><i className="fa-solid fa-key" aria-hidden /></div>
          <div className="u-stat-body">
            <div className="u-stat-label">Mots-clés surveillés</div>
        ...
        <div className="u-stat-card">
          ...
          <div className="u-stat-body">
            <div className="u-stat-label">Secteurs & Régions</div>
            <div className="u-stat-number">{sectors.length + regions.length}</div>
            <div className="u-stat-sub amber"><i className="fa-solid fa-map-location-dot" aria-hidden /> Filtres géographiques actifs</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div className="u-table-section">
          <div className="u-table-header">
            <div className="u-table-title">
              <i className="fa-solid fa-key" aria-hidden /> Alertes par mots-clés
            </div>
          ...
          <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', background: 'var(--bg)' }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <div className="u-search-input-wrap" style={{ flex: 1 }}>
                ...
                <input
                  type="text"
                  placeholder="Ex: informatique, travaux, réseau..."
                  value={newKeyword}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div className="u-table-section">
            ...
            {sectors.length === 0 ? (
              <div style={{ padding: '20px', textAlign: 'center', color: 'var(--muted)', fontSize: 12 }}>Aucun secteur configuré</div>
            ) : (
          ...
          <div className="u-table-section">
            <div className="u-table-header">
              <div className="u-table-title">
                <i className="fa-solid fa-map-location-dot" aria-hidden /> Alertes par région
              </div>
            ...
            {regions.length === 0 ? (
              <div style={{ padding: '20px', textAlign: 'center', color: 'var(--muted)', fontSize: 12 }}>Aucune région configurée</div>
            ) : (
              <div style={{ padding: '6px 0' }}>
                {regions.map((r) => (
                  <div key={r.id} style={{ display: 'flex', alignItems: 'center', padding: '8px 20px', borderBottom: '1px solid var(--border)', gap: 10 }}>
        ...
        <div className="u-search-body">
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', border: '1.5px solid var(--border)', borderRadius: 8, background: emailNotif ? '#F0FDF4' : 'var(--white)', borderColor: emailNotif ? '#BBF7D0' : 'var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>Récapitulatif selon la fréquence configurée</div>
                </div>
              </div>
              <div
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', border: '1.5px solid var(--border)', borderRadius: 8, background: browserNotif ? '#F0FDF4' : 'var(--white)', borderColor: browserNotif ? '#BBF7D0' : 'var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 2 }}>Alertes push en temps réel</div>
                </div>
              </div>
              <div
          ...
          <div style={{ marginTop: 16, display: 'flex', justifyContent: 'flex-end' }}>
            <button className="u-btn-search">
              <i className="fa-solid fa-floppy-disk" aria-hidden /> Enregistrer les préférences
            </button>
          </div>
        </div>
      </div>
